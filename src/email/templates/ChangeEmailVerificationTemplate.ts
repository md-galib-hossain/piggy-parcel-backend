...
export class ChangeEmailVerificationTemplate extends BaseEmailTemplate {
	...
	render(data: ChangeEmailVerificationData): EmailData {
		return this.buildEmail(data);
	}

	protected getEmailContent(data: ChangeEmailVerificationData): {
		subject: string;
		bodyHtml: string;
	} {
		const { userName, verificationLink, newEmail, validDuration } = data;
		const subject = `Verify Your New Email Address for ${this.appName}`;

		const bodyHtml = `
      <div style="text-align: center; margin-bottom: 30px;">
        <h2 style="color: #333; margin: 0 0 20px 0; font-size: 24px;">Email Change Verification</h2>
        <p style="color: #666; font-size: 16px; line-height: 1.6; margin: 0 0 20px 0;">
          Hello ${userName},<br/>
          You've requested to change your email address to <strong>${newEmail}</strong>.<br/>
          Please click the button below to verify this change:
        </p>
